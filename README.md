...
- **Stabilize camera motion** is a file-only sequence stage for small table or fluoroscope bumps. It estimates bounded frame translations against an early reference from high-frequency texture in multiple camera-field regions, requires spatial consensus so localized contrast flow is not treated as camera motion, and leaves low-confidence or subpixel-isolated frames unchanged. Place it before temporal denoising and ROI extraction so downstream masks and analyses share the stabilized coordinate system.
