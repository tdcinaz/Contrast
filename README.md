...
- **Frame workers** are shared by both videos and every enhancement stage. The pool grows on demand, bounds queued work to control memory use, and defaults to half of the CPUs available to the process while each OpenCV operation uses one thread. Set `CONTRAST_FRAME_WORKERS` to a positive integer to apply a worker cap established by profiling on another machine.
