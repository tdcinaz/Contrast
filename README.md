...
- **Brightness-coded contrast segmentation** uses adaptive local thresholding and connected-component filtering to identify dark contrast-filled regions. Each retained component starts with its median grayscale brightness rather than a binary foreground value. **Brightness tolerance** groups component medians within the selected maximum grayscale range so small level differences do not fragment one visual class; `0` preserves exact levels, while larger values merge a wider range. Its neighborhood, sensitivity, tolerance, and minimum-area controls can be tuned per video set. The stage captures its component map from the cumulative frame at its current pipeline position, then passes that frame through unchanged so downstream enhancement and ROI analysis are not altered.
