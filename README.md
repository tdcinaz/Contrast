...
- **Temporal pixel-change heatmap** accumulates the absolute difference between consecutive enhanced frames for every pixel. Enable **Show pixel change** beside **Show source** to display it beside the enhanced video; the two display modes are mutually exclusive. Color represents total change over the video and brightness represents mean change per second. In comparison mode, both heatmaps share the strongest total-change and per-second-rate values as their display maxima, so colors can be compared directly. It is a terminal, file-only stage and does not require an ROI mask.
